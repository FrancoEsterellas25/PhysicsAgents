...
def correr_simulacion(params, N_agentes=400, t_max=15):
    # Fijar semilla estocástica global para esta corrida
    np.random.seed(42)
    rho, lam, w1, delta_ext, tau_max, ell = params
...
def main():
    ...
    problem = {
        'names': [
            'Cópula (rho)', 'Pendiente Letalidad (lam)', 'Peso Estrés (w1)', 
            'Decaimiento Viral (delta_ext)', 'Tolerancia (tau_max)', 'Radio Aerosol (ell)'
        ],
        'bounds': [
            [0.2, 0.8], [3.0, 7.0], [0.3, 0.7], 
            [0.5, 2.5], [0.5, 10.0], [0.5, 3.0]
        ]
    }
